...
def bootstrap():
    ...
    ksk_name = isctest.run.cmd([keygen] + f"-a ECDSA256 -f KSK {zonename}".split(), cwd="ns2").stdout.decode("utf-8").strip()
    zsk_name = isctest.run.cmd([keygen] + f"-a ECDSA256 {zonename}".split(), cwd="ns2").stdout.decode("utf-8").strip()
    ...
    isctest.run.cmd(
        [signer] + f"-P -o {zonename} -f {zonename}signed.db -3 A1B2C3D4 -H too-many -H 151 -S {zonename}db".split(),
        cwd="ns2",
    )
